...
PLATFORMS: List[Platform] = []  # no select platform
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    async def add_magnet(call: ServiceCall) -> None:
        """
        Non-auth submit to qBittorrent.
        If a category is provided and base_path is set, ensure the category's savePath first,
        then add the torrent with category and savepath.
        """
        data = call.data or {}
        ...
        if category and base_path:
            ...
            if not (savepath.endswith("/") or savepath.endswith("\\")):
                savepath = f"{savepath}/"
            # Create/ensure category with savePath (non-auth is fine when qB WebUI has auth disabled)
            try:
        ...
        try:
            async with session.post(f"{base}/api/v2/torrents/add", data=form, timeout=20) as resp:
